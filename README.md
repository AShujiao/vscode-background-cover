...
1. **💾 在线图片缓存上限**：新增「图片缓存上限」设置（高级 → 自动随机，默认 200 个文件），超出后按最近使用时间自动清理最旧的缓存文件，缓存目录不再无限增长；同一张图重复下载时直接复用已有缓存，不重复占用磁盘。
2. **⚡ 粒子特效帧率节流**：新增「帧率」设置（装饰 → 帧率，默认 60 帧），高刷新率屏幕（120/144/240Hz）下不再按屏幕刷新率重绘，修复开启特效后 GPU 占用被拉满的问题（[#230](https://github.com/AShujiao/vscode-background-cover/issues/230)）。
...
#### ver 3.6.2 (2026/09/08)

1. 新增「图片缓存上限」设置（高级 → 自动随机，默认 200 个文件）：在线图片缓存超出上限后按最近使用时间自动清理最旧的缓存文件，并顺带清理下载中断遗留的临时文件，修复无扩展名 / 动态在线源长期自动换图导致缓存目录无限增长的问题（[#233](https://github.com/AShujiao/vscode-background-cover/issues/233)）。
2. 优化在线图片下载失败时的兜底：复用同一地址此前已下载的缓存文件，动态图片源偶发网络失败时不再丢失背景。
3. 动态在线源改为按图片内容去重缓存：同一张图被重复下载时直接复用已有缓存文件，不再重复占用磁盘。
4. 粒子特效增加帧率节流（默认 60 帧，可在装饰 → 帧率调整）：高刷新率屏幕下不再按屏幕刷新率重绘，修复特效开启后 GPU 占用被拉满的问题（[#230](https://github.com/AShujiao/vscode-background-cover/issues/230)）；窗口不可见时停止绘制，系统开启「减少动态效果」时自动降到 1 帧/秒。
