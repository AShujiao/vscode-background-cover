...
4. **🧩 AgentView 支持**：支持 VS Code AgentView / Agent Sessions 独立窗口背景显示。
5. **🛠️ 热更新更稳定**：优化 CSS 写入、刷新触发与并发控制，快速切换背景时不再被旧任务覆盖。
6. **✨ 在线随机图优化**：手动刷新和自动轮播会重新获取远程图片，随机图 API 不再长期复用旧缓存。
7. **📦 包体积优化**：排除 webview 开发依赖和源码，VSIX 包体大幅减小。
8. **🍎 macOS 修复**：修复赞助作者按钮无法打开的问题。
...
5. 优化背景热更新、在线随机图缓存与快速切换并发控制。
6. 修复 macOS 下赞助作者按钮无法打开的问题。
7. 优化 VSIX 打包配置，显著减小包体。
