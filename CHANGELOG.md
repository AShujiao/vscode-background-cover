...
#### ver 3.7.0 (2026/09/12)
    ...
    1. 背景切换过渡动画：换图时旧图渐隐、新图渐显，平滑不闪屏，支持自动换图 / 手动换图；换图前自动预加载，消除首帧闪烁（尊重系统「减少动态效果」）。
    2. 新增「补丁状态自检」：补丁丢失时自动提示重新应用（借鉴 [shalldie/vscode-background](https://github.com/shalldie/vscode-background) 状态机）。
    3. 「安装似乎损坏」提示自动屏蔽：内置 16 种语言规则，首次安装无需手动点「不再提示」。
    4. 混合模式 auto 改为 CSS 变量 + `:has()` 主题感知：切换深浅主题即时生效。
    5. 背景源支持 `~`、`${ENV}`、`$ENV` 路径变量与文件夹。
    6. 修复 `blendModel` 默认值不在枚举内（`cover` → `auto`）。
    7. 修复自动随机「到点有事件但不换图」：清理过期在线单图记录，配置本地随机目录时优先本地轮换。

#### ver 3.6.2 (2026/09/08)

    1. 新增「图片缓存上限」设置（高级 → 自动随机 → 图片缓存上限，默认 200 个文件）：在线图片缓存超出上限后按最近使用时间自动清理最旧的缓存文件，并顺带清理下载中断遗留的临时文件，修复无扩展名 / 动态在线源长期自动换图导致缓存目录无限增长的问题（[#233](https://github.com/AShujiao/vscode-background-cover/issues/233)）。
    2. 优化在线图片下载失败时的兜底：复用同一地址此前已下载的缓存文件，动态图片源偶发网络失败时不再丢失背景。
    3. 动态在线源改为按图片内容去重缓存：同一张图被重复下载时直接复用已有缓存文件，不再重复占用磁盘（图池型 / 内容稳定的在线源收益最明显）。
    4. 粒子特效增加帧率节流（默认 60 帧，可在装饰 → 帧率调整）：高刷新率屏幕（120/144/240Hz）下不再按屏幕刷新率重绘，修复特效开启后 GPU 占用被拉满的问题（[#230](https://github.com/AShujiao/vscode-background-cover/issues/230)）；窗口不可见时停止绘制，系统开启「减少动态效果」时自动降到 1 帧/秒。
