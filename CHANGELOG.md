...
#### ver 3.5.0 (2026/05/23)
    ...
    5. 顶部小宠物配置统一到 Studio，并扩展小宠物列表与资源预览。
    6. 优化背景热更新底层：跳过未变化 CSS 写入、增强状态栏触发失败时的刷新兜底、避免快速切换时旧异步任务覆盖新背景。
    7. 优化在线随机图缓存策略，手动刷新与自动轮播会重新获取远程图片，避免随机图 API 长期复用旧缓存。
    8. 修复 macOS 下赞助作者按钮无法打开的问题。
    9. 优化打包配置，排除 webview 开发依赖和源码，VSIX 包体显著减小。
