...
#### ver 3.6.2 (2026/09/08)
    ...
    1. 新增「图片缓存上限」设置（高级设置 → 自动随机 → 图片缓存上限，默认 200 个文件）：在线图片缓存超出上限后按最近使用时间自动清理最旧的缓存文件，并顺带清理下载中断遗留的临时文件，修复无扩展名 / 动态在线源长期自动换图导致缓存目录无限增长的问题（[#233](https://github.com/AShujiao/vscode-background-cover/issues/233)）。
